...
class OfficeService:
    ...
    def __init__(self): self.repo = OfficeRepository()
    ...
    async def index(self, db: AsyncSession):
        ...
        for row in temp_records:
            records.append({
                "id": fernet_encrypt(str(row.office_id)).decode(), 
                "code": row.code, 
                "name": row.name, 
                "address": row.address,
            })
    ...
    async def insert(self, db: AsyncSession, form_request: OfficeInsert):
        ...
        obj = Office(**form_request.model_dump()) 
        office_id = await self.repo.insert(db, obj) 
    ...
    async def update(self, db: AsyncSession, id: str, form_request: OfficeUpdate):
        ...
        office_id = await self.repo.update(db, obj)
